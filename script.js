...
const sfItems = document.querySelectorAll('.sf-item');
const sfTitle = document.getElementById('sfTitle');
const sfDesc = document.getElementById('sfDesc');
const sfPanelInner = document.getElementById('sfPanelInner');

function setActiveFeature(item) {
  sfItems.forEach(i => i.classList.remove('active'));
  item.classList.add('active');
  const title = item.getAttribute('data-title');
  const desc = item.getAttribute('data-desc');
  const icon = item.getAttribute('data-icon');
  if (hasGSAP && !reduceMotion) {
    gsap.to(sfPanelInner, { opacity: 0, y: 10, duration: 0.2, onComplete: () => {
      sfTitle.textContent = title;
      sfDesc.textContent = desc;
      sfPanelInner.querySelector('.sf-mark i').className = 'ti ' + icon;
      gsap.to(sfPanelInner, { opacity: 1, y: 0, duration: 0.3 });
    }});
  } else {
    sfTitle.textContent = title;
    sfDesc.textContent = desc;
...
sfItems.forEach(item => {
  item.addEventListener('click', () => setActiveFeature(item));
});

// Auto-advance on scroll through the section (desktop only)
if (hasGSAP && sfItems.length && window.innerWidth > 860 && !reduceMotion) {
  const sfSection = document.querySelector('.sticky-features');
  ScrollTrigger.create({
    trigger: sfSection,
    start: 'top 30%',
    end: 'bottom 60%',
    onUpdate: (self) => {
      const idx = Math.min(sfItems.length - 1, Math.floor(self.progress * sfItems.length));
      const target = sfItems[idx];
      if (target && !target.classList.contains('active')) setActiveFeature(target);
    }
  });
}
